import gradio as gr
from youdub.step000_video_downloader_x import download_from_url
from youdub.step010_demucs_vr import separate_all_audio_under_folder
from youdub.step020_whisperx_zh import transcribe_all_audio_under_folder
from youdub.step030_translation_x import translate_all_transcript_under_folder
from youdub.step040_tts_x import generate_all_wavs_under_folder
...
do_everything_interface = gr.Interface(
    fn=do_everything,
    inputs=[
        gr.Textbox(label='Root Folder', value='videos_zh2en'),  # Changed 'default' to 'value'
        gr.Textbox(label='Video URL', placeholder='Video or Playlist or Channel URL',
                   value='https://www.bilibili.com/video/BV1Hy411B7SJ'),  # Changed 'default' to 'value'
        ...
        gr.Radio(['distil-large-v2', 'distil-medium.en', 'large', 'medium', 'small', 'base', 'tiny'], label='Whisper Model', value='large'),
        ...
        gr.Dropdown(['简体中文', '繁体中文', 'English', 'Deutsch', 'Français', 'русский'],
                    label='Translation Target Language', value='English'),
...
youtube_interface = gr.Interface(
    ...
    inputs=[
        gr.Textbox(label='Video URL', placeholder='Video or Playlist or Channel URL',
                   value='https://www.bilibili.com/video/BV1Hy411B7SJ'),  # Changed 'default' to 'value'
        gr.Textbox(label='Output Folder', value='videos_zh2en'),  # Changed 'default' to 'value'
        gr.Radio(['4320p', '2160p', '1440p', '1080p', '720p', '480p', '360p', '240p', '144p'], label='Resolution', value='1080p'),
...
demucs_interface = gr.Interface(
    ...
    inputs = [
        gr.Textbox(label='Folder', value='videos_zh2en'),  # Changed 'default' to 'value'
...
whisper_inference = gr.Interface(
    ...
    inputs = [
        gr.Textbox(label='Folder', value='videos_zh2en'),  # Changed 'default' to 'value'
        gr.Radio(['distil-large-v2', 'distil-medium.en', 'large', 'medium', 'small', 'base', 'tiny'], label='Model', value='large'),
        gr.Textbox(label='Download Root', value='models/ASR/whisper'),
        gr.Radio(['auto', 'cuda', 'cpu'], label='Device', value='auto'),
        gr.Slider(minimum=1, maximum=128, step=1, label='Batch Size', value=16),
