...
def valid_translation(text, translation):
    ...
    if 'Translation' in translation and ': "' in translation and '"' in translation:
        translation = translation.split(': "')[-1].split('"')[0]
        return True, translation_postprocess(translation)
...
def _translate(summary, transcript, target_language='简体中文'):
    ...
    # 如果目标语言没有定义，则默认为中文
    if target_language not in example_translations:
        target_language = '简体中文'

    # 构建固定消息，加入相应的示例翻译
    fixed_message = [
        {'role': 'system', 'content': f'You are an expert in the field of this video.\n{info}\nTranslate the sentence into {target_language}.'},
        {'role': 'user', 'content': f'Translate: "{example_translations[target_language][0][0]}"'},
        {'role': 'assistant', 'content': f'Translation: "{example_translations[target_language][0][1]}"'},
        {'role': 'user', 'content': f'Translate: "{example_translations[target_language][1][0]}"'},
        {'role': 'assistant', 'content': f'Translation: "{example_translations[target_language][1][1]}"'},
    ]
    ...
    for line in transcript:
        ...
        for retry in range(30):
            ...
            logger.info(f'翻译请求messages：{messages}')
