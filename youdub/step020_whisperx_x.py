...
def transcribe_audio(folder, model_name: str = 'large', download_root='models/ASR/whisper', device='auto', batch_size=32, diarization=True,min_speakers=None, max_speakers=None):
    ...
    with open(os.path.join(folder, 'whisperx.align.rec_result.json'), 'w', encoding='utf-8') as f:
        json.dump(rec_result, f, indent=4, ensure_ascii=False)
    ...
    with open(os.path.join(folder, 'whisperx.assign_word_speakers.rec_result.json'), 'w', encoding='utf-8') as f:
        json.dump(rec_result, f, indent=4, ensure_ascii=False)
